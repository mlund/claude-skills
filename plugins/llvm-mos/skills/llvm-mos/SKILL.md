...
These rules come from the project's own optimization guide; the reasoning matters more than the list. Byte and cycle figures below were taken at `-Oz` with the default LTO unless stated otherwise; `-Os` moved them by single digits where both were measured.
...
- **Prefer structs of arrays.** Absolute-indexed addressing needs a constant base plus an 8-bit index; an array of structs needs a multiply by the stride. That is worse than "a multiply" sounds — a non-power-of-two stride emits a *call*, and drags the routine into the link:
  ...
  Padding the struct to 8 or 16 bytes turns that into `asl`/`rol` pairs, which is the escape hatch if you must keep the struct — at the price of the padding. `#include <soa.h>` provides `soa::Array<T, N>` in C++ which does the byte-striping for you while looking like a normal array; LTO mixes C and C++ freely, so reaching for it does not mean rewriting the project. Measured on a 64-entry array of a 3-byte struct, summing two fields per entry: **2913 cycles against 3560** for the plain array of structs.

  **Its whole benefit is conditional on the address being a link-time constant, and it becomes a pessimization when that fails.** The striped layout only pays because `base + N*k` folds into absolute indexed addressing, so the array must be a global or static, the element type must be trivial and standard-layout with alignment 1, and `N` is a `uint8_t` — 255 entries maximum. Reach it through a pointer instead and the addressing reverts to 16-bit pointer arithmetic: the same loop behind a genuine runtime pointer measured **4912 cycles**, worse than the plain C array it replaced. Forming pointers into element contents, or storing and passing the element proxies, does the same thing — the header warns those can take on their logical representation, one pointer per byte.

  That failure is easy to miss in a small test. Passing `&arr` to a single `noinline` function measured *faster* (2803 cycles), because LTO propagated the one call site's constant and recovered absolute addressing. The pessimization only appeared once the pointer became genuinely dynamic, so a microbenchmark with one call site will tell you it is fine when it is not.
...
- **`printf` is the most expensive thing most programs link, and its float conversions silently do nothing.** A `printf("%u\n", v)` measured **4535 bytes** against **515** for a `putchar` loop that formats the same integer — 4020 bytes for one call. Worse, the default `printf` is built without float support, so `printf("%f\n", d)` prints the literal text `%f`: no compile error, no link error, no warning, just a format string where a number should be. Link `-lprintf_flt` to get `3.140000` instead, at **+5400 bytes** (4606 → 10006 on the same program). That cost is why it is opt-in, and it is the reason to reach for scaled integers and a hand-rolled formatter.
- **There is no FPU, and `<fixed_point.h>` is the SDK's answer.** `FixedPoint<Int, Frac, Signed>` with aliases `f8_8`, `f16_16`, `f24_8` and friends behaves like an integer type that carries its own implicit divisor, so same-format addition is plain 16- or 32-bit math and the integer part of an 8.8 value is just its high byte. Measured against `float` on the same 50-iteration multiply-accumulate producing the same answer: **4714 bytes and 7469 cycles against 11332 and 260172** — about 35× the speed for 6618 fewer bytes. The `fixedpoint_literals` suffixes (`0.5_16_16`) are `consteval` and the float constructor is `explicit`, so a decimal in your source is converted at compile time and no runtime float can leak in by accident. It is a C++ header, which under LTO costs nothing to mix into a C project. `<math.h>` is a stub, so the alternative to this is usually writing the arithmetic yourself.
...
- **Touching `malloc` at all links the whole allocator** — a single `malloc`/`free` pair measured **1606 bytes against 54** for the same program without it. The heap is the one part of the library that costs nothing until first use, so a stray allocation in a rarely-taken path is 1.5 KB you may not know you are carrying. Its base also sits directly above `.bss` (the header's memory map is the reference), which means every global you add pushes the heap base up and takes that space out of the heap rather than out of some separate pool.
- **A `longjmp` rolls non-volatile locals back to their values at `setjmp`, and the answer changes with optimization level.** C has always said such objects are indeterminate; here the concrete behaviour is a silent revert, because `setjmp` saves `__rc18`–`__rc31` into the `jmp_buf` and `longjmp` restores them, taking any local the allocator parked there back with it. A local set to 11 after `setjmp` read back as **10 at `-O1`, `-Os` and `-Oz`, but 11 at `-O0`** — so the bug appears when you turn optimization on and vanishes when you go looking for it in a debug build. `-Wall -Wextra` says nothing. Anything that must survive a `longjmp` needs to be `volatile`, `static`, or global; the `volatile` local in the same test kept its new value at every level.
